...
.env                 optional Relay pairing token (presence-gates section 14), mail-plane credentials (schema: docs/configuration.md "Mail plane"); typed-dispatch TYPESAFE_API_KEY or TYPESAFE_API_PROVIDER=openrouter (Keychain key; gates bin/fm-dispatch-resolve.sh; docs/configuration.md "Typed dispatch resolution"); LOCAL, gitignored
...
config/no-mistakes-auto  optional one-token answer to whether a finished no-mistakes task validates automatically: absent or "on" always runs no-mistakes, "off" defers the run-or-skip choice to the captain after implementation; LOCAL, gitignored; written only by bin/fm-validation-decision.sh (Pi: /toggle-no-mistakes); inherited by secondmate homes; see docs/configuration.md "Automatic no-mistakes validation"
...
The one exception is a task whose record defers that choice (`validation_decision=pending`, which the scaffold, spawn, and promotion scripts record themselves from `config/no-mistakes-auto`): its worker stops at a captain decision point instead, so load `validation-decision` before triggering validation or relaying any answer, and never skip no-mistakes on anything but the captain's own current words.
...
- `validation-decision` - load when a worker reports `needs-decision [key=validation-decision]`, when a task's record reads `validation_decision=pending` at recovery or current-state reconciliation, before recording or relaying the captain's run-or-skip no-mistakes answer, and when the captain asks to turn automatic no-mistakes on or off outside Pi's `/toggle-no-mistakes`.
