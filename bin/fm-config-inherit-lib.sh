...
# Primary config/no-mistakes-auto is the captain's standing answer to whether a
# finished no-mistakes task validates automatically or waits for the captain's
# run-or-skip decision (bin/fm-validation-decision.sh), so it flows down too:
# a secondmate's own crewmates stop at the same decision point, and that
# decision reaches the captain over the parent channel like any other held call.
# It also pushes
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode no-mistakes-auto}"
